...
**Zawężenie po pomiarze (DEC-27).** Zdanie „zero naruszeń dowodzi, że nic się nie złamało w oknie" jest
prawdziwe wyłącznie wobec ruchu, który konfiguracja dry-run w ogóle **ocenia**. Ruch między dwoma członkami
będącymi oba w dry-run jest dla niej ruchem wewnątrz perimetru i nie może dać ani jednego wpisu — a po
promocji jednego z nich staje się naruszeniem egress. Okno mierzy więc ruch przez granicę **dzisiejszą**,
a promocja przesuwa granicę; różnica tych dwóch zbiorów jest niemierzalna z definicji i dlatego jest
DEKLAROWANA (`unmeasured_peers_ack`), a nie mierzona.
...
## DEC-25 — Weryfikacja egressu wymaga maszyny WEWNĄTRZ perimetru; „bez maszyny" dotyczy wyłącznie ingressu
...
## DEC-35 — Polityka alertu i deskryptor jej metryki należą do TEGO SAMEGO stacku
...
trzy produkcyjne nietknięte. Procedura, dwie zmierzone pułapki (`invalid_rapt` — Terraform bierze ADC,
nie sesję `gcloud`; warstwa Deny wymaga `roles/iam.denyAdmin`) i koszt sprzątania: `1-wdrozenie.md`
§Próba generalna.

Czego ten dowód **nie** obejmuje: warstwy Deny — próba jechała z `manage_deny_policy = false`, bo
tożsamość prowadząca nie miała `roles/iam.denyAdmin`. Różnicowanie nazwy polityki
(`vpcsc-ci-no-destroy-{dra,drb}`) zostało zmierzone na wyrenderowanym planie, nie na żywym obiekcie.
