...
| custom rola `vpcScSinkReader` (gdy `grant_sink_reader`) | **jedno** uprawnienie `logging.sinks.get` na org — guard raportu naruszeń sprawdza, że sink istnieje, ma `includeChildren` i ten sam filtr co odczyt. Zakres org-level wynika z tego, że sink org-level nie ma innego rodzica; czyta **konfigurację** logowania, nie treść logów. Zastąpiła org-wide `roles/logging.viewer` (28 uprawnień, w tym odczyt treści każdego logu w organizacji) |
