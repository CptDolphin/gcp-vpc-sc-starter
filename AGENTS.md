...
1. [`docs/0-decyzje.md`](docs/0-decyzje.md) — rejestr decyzji, każda z odrzuconymi wariantami. Komentarze
   w kodzie odsyłają tam skrótem `(DEC-4)`. **Bez tego pliku połowa kodu wygląda na przekomplikowaną.**
   Liczby decyzji nie ma tu celowo: stała tu przez pół roku i była nieprawdziwa, bo nic jej nie mierzyło
   (DEC-20 — dziś mierzy ją bramka, więc wpisana liczba czerwieni się w dniu, w którym przestaje być prawdą).
...
| **Każdy** zasób granularny ma w grafie ścieżkę do `…service_perimeter.this` | `members.tf` + `rules.tf` (6 × `depends_on`) + asercja selftestu pytająca `terraform graph` o **osiągalność**, w obu wariantach `manage_skeleton` | perimetr też jest referowany po NAZWIE (`local.perimeter_full_name`), więc bez tych pozycji Terraform tworzy członków i reguły RÓWNOLEGLE ze szkieletem: `Plan: 20 to add` → `Error 404: Service perimeter not found` ×4, a szkielet po takim przebiegu **nie istnieje**. Widać to wyłącznie przy odtwarzaniu granicy od zera — czyli nigdy w stanie ustalonym (DEC-35). Asercja pyta o ŚCIEŻKĘ, nie o krawędź: `terraform graph` drukuje graf po redukcji przechodniej, więc dwie z sześciu krawędzi nie są rysowane |
| Bramka kasowania pyta o REFEROWANIE poziomu, nie o sam fakt usunięcia | reguły `deny` w `perimeter.rego` (osobno perimetr, osobno access level) + 5 testów rego | te dwa obiekty przez pół roku były zrównane, więc bramka odrzucała offboarding dywizji z własnym access levelem — czyli **jedyną** ścieżkę, dla której istnieje `depends_on` wiersz wyżej. Dwa niezmienniki tego samego repozytorium mówiły coś przeciwnego, a różnica wychodziła wyłącznie przy wyprowadzaniu członka (DEC-33) |
...
| **Każdy** katalog z `*.tf` ma komplet bramek bezkredencjałowych (`fmt`, `init -backend=false`, `validate`, `tflint --config`) **i własną ścieżkę w wyzwalaczu** `validate.yml` | `stacki_terraform()` w selfteście — lista stacków wyprowadzona z DRZEWA repo, potem odpytanie workflow o kroki i ścieżkę dla każdego z nich (DEC-34) | asercja licząca wystąpienia (`== 2`) mierzy, czy ktoś zepsuł to, co już jest, i jest ślepa na to, czego nie ma — nowy stack przechodzi obok, a licznik zostaje zielony. Zmierzone dwa razy: `iam-bootstrap` miał lintera bez `fmt`/`validate`, a `violations-sink` nie miał NIC i przepuścił błąd składni HCL przez cały pipeline bez ani jednego czerwonego joba (2026-08-13). Kosztem nie jest czerwony PR po fakcie: ten stack applikuje **człowiek** z org-level `roles/logging.configWriter`, którego konto pipeline'u świadomie nie ma, więc błąd wychodzi dopiero w ręcznej sesji osoby o najwyższych uprawnieniach w organizacji |
| **Werdykt bramki jest odróżnialny od awarii bramki bez czytania logu** | krok `werdykt bramek tresci` (`if: always()`) w `.github/actions/bramki-tresci` — klasyfikuje z DWÓCH znaczników pisanych przez kroki, które naprawdę się wykonały; trzy stany świata uruchamiane w selfteście | zmierzone: krok instalujący `conftest` padł na pobraniu, WSZYSTKIE bramki poniżej dostały `outcome=skipped`, a check wyglądał identycznie jak odrzucenie wniosku przez regułę. Kolor przebiegu nie niesie werdyktu — nośnikiem jest adnotacja (`::error title=…`), bo kroków WEWNĄTRZ akcji złożonej REST API nie wystawia w ogóle (DEC-26) |
